...
| -------------------------- | ----------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **`<xsl:attribute>`**      | Añade un atributo a un elemento HTML o XML en el resultado.                               | `<a href="enlace.html">`<br><br>  <br><br>`<xsl:attribute name="target">_blank</xsl:attribute>`<br><br>  <br><br>`Link`<br><br>  <br><br>`</a>`                                  |
| **`<xsl:choose>`**         | Estructura condicional múltiple (funciona como un _switch_ o _if-else_).                  | `<xsl:choose>`<br><br>  <br><br>`<xsl:when test="@nota &gt;= 5">Aprobado</xsl:when>`<br><br>  <br><br>`<xsl:otherwise>Suspenso</xsl:otherwise>`<br><br>  <br><br>`</xsl:choose>` |
| **`<xsl:decimal-format>`** | Configura cómo se formatearán los números (separador de miles, decimales, etc.).          | `<xsl:decimal-format name="es" decimal-separator="," grouping-separator="."/>`<br><br>  <br><br>_(Se usa luego con `format-number()`)_                                           |
| **`<xsl:for-each>`**       | Crea un bucle para repetir instrucciones por cada nodo encontrado.                        | `<xsl:for-each select="//alumno">`<br><br>  <br><br>`<li><xsl:value-of select="nombre"/></li>`<br><br>  <br><br>`</xsl:for-each>`                                                |
| **`<xsl:if>`**             | Condicional simple. Ejecuta el código interior si la condición es verdadera.              | `<xsl:if test="stock = 0">`<br><br>  <br><br>`<span>Agotado</span>`<br><br>  <br><br>`</xsl:if>`                                                                                 |
| **`<xsl:import>`**         | Trae e integra las reglas de otra hoja de estilos externa.                                | `<xsl:import href="funciones_globales.xsl"/>` _(Debe ir siempre al principio del documento)_                                                                                     |
| **`<xsl:key>`**            | Crea un índice/clave para buscar nodos rápidamente (como un index de base de datos).      | `<xsl:key name="buscarPorDni" match="persona" use="dni"/>`<br><br>  <br><br>_(Se consulta usando `key('buscarPorDni', '12345678X')`)_                                            |
| **`<xsl:output>`**         | Define el formato del documento final generado.                                           | `<xsl:output method="html" encoding="UTF-8" indent="yes"/>`                                                                                                                      |
| **`<xsl:preserve-space>`** | Evita que el procesador elimine los espacios en blanco de ciertos tags.                   | `<xsl:preserve-space elements="codigo poema texto_libre"/>`                                                                                                                      |
| **`<xsl:sort>`**           | Ordena los nodos de un bucle o plantilla alfabética o numéricamente.                      | `<xsl:for-each select="producto">`<br><br>  <br><br>`<xsl:sort select="precio" data-type="number" order="descending"/>`<br><br>  <br><br>`</xsl:for-each>`                       |
| **`<xsl:strip-space>`**    | Elimina los espacios en blanco innecesarios de los tags indicados para limpiar la salida. | `<xsl:strip-space elements="*"/>` _(Limpia los espacios de todos los elementos)_                                                                                                 |
| **`<xsl:template>`**       | Bloque fundamental que define qué hacer cuando se encuentra un nodo específico.           | `<xsl:template match="/instituto/curso">`<br><br>  <br><br>`<h2>Curso: <xsl:value-of select="@nombre"/></h2>`<br><br>  <br><br>`</xsl:template>`                                 |
| **`<xsl:value-of>`**       | Extrae el texto de un nodo o el resultado de una expresión y lo escribe en la salida.     | `<p>Nombre: <xsl:value-of select="user/nombre"/></p>`                                                                                                                            |
| **`<xsl:variable>`**       | Almacena un valor fijo en memoria para poder reutilizarlo usando el símbolo `$`.          | `<xsl:variable name="iva" select="0.21"/>`<br><br>  <br><br>`Total: <xsl:value-of select="subtotal * $iva"/>`                                                                    |
